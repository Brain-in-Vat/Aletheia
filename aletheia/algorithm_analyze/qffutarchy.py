                        ...
                        [
                            dbc.Card(
                                [
                                    dbc.CardBody(
                                        [
                                            html.H4("The Biggest Loss Experiment Config",
...
def create_detail_fig(data):
    ...
    df_dict = {
        ...
        'Token': [x for x in gene[agent_numer: agent_numer * 2]],
        'Agent': [x for x in range(agent_numer)]
    }

    data_df = pd.DataFrame(data=df_dict)
    # data_fig = px.scatter(data_df, x='belief', y='tokens')
    data_fig = px.scatter(data_df, x='Agent', y='Belief',
                          size='Token', size_max=60, title='Distribute of Belief and Tokens')
    ...
    for clr_amount in clr_amounts:
        clr_df_dict['Project Id'].append(clr_amount['id'])
        clr_df_dict['Grant Amount'].append(clr_amount['clr_amount'])
        clr_df_dict['Number of Contributions'].append(
            clr_amount['number_contributions'])
        clr_df_dict['Contribution Amount'].append(
            clr_amount['contribution_amount'])

    clr_df = pd.DataFrame(data=clr_df_dict)
    # clr_fig = px.bar(clr_df, x='id', y='clr_amount')
    clr_fig = px.bar(clr_df, x='Project Id', y=[
                     'Grant Amount', 'Contribution Amount'], title='Distribution of Grant Amount and Contribution Amount')
    ...
    for clr_amount in clr_amounts:
        ctr_number_dict['Project Id'].append(clr_amount['id'])
        ctr_number_dict['Number of Contributions'].append(
            clr_amount['number_contributions'])

    ctr_number_df = pd.DataFrame(data=ctr_number_dict)
    ctr_number_fig = px.bar(ctr_number_df, x='Project Id', y='Number of Contributions',
    ...
    for index, value in enumerate(token_changed):
        ...
        token_changed_data['Agent Id'].append(index)
        token_changed_data['Token Changes'].append(value)

    token_changed_df = pd.DataFrame(data=token_changed_data)
    token_changed_fig = px.scatter(
        token_changed_df, x='Agent Id', y='Token Changes', title='Dsitribution of Token Benefit')

    page_layout = html.Div([
        # html.H1('Agent Distribute'),
        dbc.Row(dbc.Col(html.H1('Visualization of Each Iteration'))),
